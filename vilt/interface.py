...
class Interface:
    ...
    def __init__(
        ...
        def get_input_instance(iface):
            if isinstance(iface, str):
                ...
                raise ValueError(
                    "Input interface must be of type `str` or " "`InputComponent`"
                )
        ...
        def get_output_instance(iface):
            if isinstance(iface, str):
                ...
                raise ValueError(
                    "Output interface must be of type `str` or " "`OutputComponent`"
                )
        ...
        if self.allow_flagging:
            ...
            while os.path.exists(
                self.flagging_dir + "/" + dir_name + "_{}".format(index)
            ):
                index += 1
            self.flagging_dir = self.flagging_dir + "/" + dir_name + "_{}".format(index)
        ...
        try:
            ...
            for i, iface in enumerate(config["output_interfaces"]):
                outputs_per_function = int(
                    len(self.output_interfaces) / len(self.predict)
                )
                ...
                if len(self.predict) > 1:
                    iface[1]["label"] = (
                        self.function_names[function_index].replace("_", " ")
                        + ": "
                        + iface[1]["label"]
                    )
    ...
    def embed(self, processed_input):
        if self.embedding == "default":
            embeddings = np.concatenate(
                [
                    input_interface.embed(processed_input[i])
                    for i, input_interface in enumerate(self.input_interfaces)
                ]
            )
        else:
    ...
    def test_launch(self):
        for predict_fn in self.predict:
            ...
            for input_interface in self.input_interfaces:
                if (
                    input_interface.test_input is None
                ):  # If no test input is defined for that input interface
                    print("SKIPPED")
                    break
    ...
    def launch(self, inline=None, inbrowser=None, share=False, debug=False):
        ...
        if share:
            print(
                "This share link will expire in 24 hours. If you need a "
            ...
            try:
                ...
                if self.analytics_enabled:
                    try:
                        requests.post(
                            analytics_url + "gradio-error-analytics/", data=data
                        )
                    except requests.ConnectionError:
                        pass  # do not push analytics if no network
                share_url = None
            ...
            print("To get a public link for a hosted model, " "set Share=True")
            if self.verbose:
        ...
        r = requests.get(
            path_to_local_server + "enable_sharing/" + (share_url or "None")
        )
